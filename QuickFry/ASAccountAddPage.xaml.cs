...
public partial class ASAccountAddPage : ContentPage
{
    AccountViewModel AccVM = new AccountViewModel();

    public ASAccountAddPage()
	{
        InitializeComponent();
    ...
    private async void RegisterClicked(object sender, EventArgs e)
    {
        ...
        if (!rbAdmin.IsChecked && !rbRider.IsChecked)
        {
            if (b)
        ...
        if (b)
        {
            ...
            if (addAccount)
            {
                ...
                if (rbAdmin.IsChecked)
                    type = "ADMIN";
                if (rbRider.IsChecked)
                    type = "RIDER";
                AccVM.loadingPopup = new MauiToolkitPopupSample._0PopupLoadingPage();
                this.ShowPopup(AccVM.loadingPopup);

                AccVM.AdminRegisterAccount(txtUser.Text, txtPassword.Text, txtName.Text, txtAddress.Text, txtContact.Text, "", type);
            }
        }
    }
}